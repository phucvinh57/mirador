...
constexpr MirInputEventModifiers whitelist_modifiers =
    mir_input_event_modifier_alt | mir_input_event_modifier_shift | mir_input_event_modifier_ctrl |
...
auto matches(Keybind const& binding, MirKeyboardEvent const* event) -> bool
{
    ...
    auto const event_modifiers =
        miral::toolkit::mir_keyboard_event_modifiers(event) & whitelist_modifiers;
    auto const allows_shift_fallback =
        binding.modifiers != 0 && !(binding.modifiers & mir_input_event_modifier_shift);
    ...
    for (auto const& [name, values] : config->search_prefix("command_")) {
        ...
        for (auto const& value : values) {
            ...
            if (separator == std::string::npos || separator == 0 || separator + 1 >= value.size()) {
                mir::log_warning("Ignoring invalid keybind entry for %s: %s", name.c_str(),
                                 value.c_str());
                continue;
            }
            ...
            if (!key) {
                mir::log_warning("Ignoring keybind with unrecognised key for %s: %s", name.c_str(),
                                 value.c_str());
                continue;
            }
...
auto KeybindConfig::handle(MirKeyboardEvent const* event, miral::MirRunner& runner,
